...
if [[ ${1} == "checkdigests" ]]; then
    ...
    app_url="http://localhost:9999"
    docker run --rm --network host -d --name service -e DEBUG="yes" -e STASH_STASH=/ -e STASH_METADATA=/ -e STASH_GENERATED=/ -e STASH_CACHE=/ "${2}"
    currenttime=$(date +%s); maxtime=$((currenttime+60)); while (! curl -fsSL "${app_url}" > /dev/null) && [[ "$currenttime" -lt "$maxtime" ]]; do sleep 1; currenttime=$(date +%s); done
    ...
    version=$(curl -u "${GITHUB_ACTOR}:${GITHUB_TOKEN}" -fsSL "https://api.github.com/repos/stashapp/stash/releases/latest" | jq -r .tag_name | sed s/v//g)
    [[ -z ${version} ]] && exit 1
    old_version=$(jq -r '.version' < VERSION.json)
    changelog=$(jq -r '.changelog' < VERSION.json)
    [[ "${old_version}" != "${version}" ]] && changelog="https://github.com/stashapp/stash/compare/v${old_version}...v${version}"
