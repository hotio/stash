...
RUN --mount=type=secret,id=GIT_AUTH_TOKEN.github.com,env=TOKEN \
    ...
    curl -fsSL ${TOKEN:+--header "Authorization: Bearer ${TOKEN}"} "https://api.github.com/repos/intel/compute-runtime/releases/tags/${VERSION_INTEL_CR}" | jq -r '.assets[].browser_download_url' | grep -e "libigdgmm.*\.deb" >> list.txt && \
    curl -fsSL ${TOKEN:+--header "Authorization: Bearer ${TOKEN}"} "https://api.github.com/repos/intel/compute-runtime/releases/tags/${VERSION_INTEL_CR}" | jq -r '.assets[].browser_download_url' | grep -e "intel-opencl-icd.*\.deb" >> list.txt && \
    curl -fsSL ${TOKEN:+--header "Authorization: Bearer ${TOKEN}"} "https://api.github.com/repos/intel/compute-runtime/releases/tags/${VERSION_INTEL_CR}" | jq -r '.body' | grep "wget" | grep "intel-graphics-compiler" | sed 's|wget ||g' >> list.txt && \
