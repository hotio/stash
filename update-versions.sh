...
version=$(curl -fsSL ${GITHUB_TOKEN:+--header "Authorization: Bearer ${GITHUB_TOKEN}"} "https://api.github.com/repos/stashapp/stash/releases/latest" | jq -re .tag_name)
version_check() {
    if [[ "${version}" != "${old_version}" ]]; then
        curl -fsSL ${GITHUB_TOKEN:+--header "Authorization: Bearer ${GITHUB_TOKEN}"} "https://github.com/stashapp/stash/releases/download/${version}/stash-linux" -o /dev/null || return 1
    fi
}
